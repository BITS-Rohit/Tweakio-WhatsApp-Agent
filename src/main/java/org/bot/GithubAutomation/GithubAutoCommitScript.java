...
public class GithubAutoCommitScript {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            System.err.println("Invalid GitHub URL: " + e.getMessage());
            return;
        }
        System.out.println("Parsed repo -> Owner: " + info.owner + ", Repo: " + info.repo + ", Branch: " + info.branch + ", Path: " + info.path);

        new GithubAutoCommitScript().run(info, info.path.isEmpty() ? "daily-update.txt" : info.path);
    }

    public void run(RepoInfo info, String filePath) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                System.out.println("✅ Commit successful: " + response.code());
            } else {
                assert response.body() != null;
                System.err.println("❌ Commit failed: HTTP "
